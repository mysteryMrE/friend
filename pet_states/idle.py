...
class IdleState(PetState):
    def handle_event(self, event_number):
        if event_number == 5:
            ...
            self._pet.set_state(
                IdleToSleepState(self._pet), called_from="IdleState.handle_event"
            )
        elif (
            ...
            self._pet.set_state(
                WalkLeftState(self._pet), called_from="IdleState.handle_event"
            )
        elif event_number in self._pet.walk_right_events:
            from pet_states.walk_right import WalkRightState

            self._pet.set_state(
                WalkRightState(self._pet), called_from="IdleState.handle_event"
            )
